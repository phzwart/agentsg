...
the invariant indexes 3000 cells in \SI{0.10}{\second} and reduces a 200-frame
serial reindexing problem from \SI{17.5}{\milli\second} to \SI{44}{\micro\second}
per frame.
...
A NearTree metric index [Andrews \& Bernstein, 2016] over 3000 perturbed cells
built in \SI{0.10}{\second}; 40/40 exact nearest-neighbour queries recovered the
planted cell at a mean \SI{0.75}{\milli\second}, and queries placed on opposite
sides of a reduction boundary returned the same reference lattice. The invariant
performs the high-volume filtering; equation~\eqref{eq:lift} is evaluated only for returned
...
frame at \SI{17.5}{\milli\second} per frame; the root-first workflow established
...
\SI{44}{\micro\second} per frame --- a $\approx 400$-fold per-frame reduction
...
than a limitation. The remaining work is validation depth, not scope: database
timing on archival-scale, multi-protein holdings; serial validation against
experimental intensities and production reindexing decisions; and trajectory
transport on measured thermal, time-resolved or operando series with uncertainty
propagation and cycle-consistency diagnostics. The central algorithmic claim is
