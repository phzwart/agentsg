...
- `wyckoff.py` — site symmetry / orbits / multiplicities, plus an exact
  ...
  fixed sets).
- `reflection_lattice.py` — reflection conditions as sublattices in
  projection: reflection classes are the stabiliser strata of the point-group
  action on reciprocal space (closure under intersection of the fixed
  subspaces of the `W`), and the present reflections of a stratum are the
  dual of the translation lattice generated by its stabiliser's operations,
  `V_S ∩ Z^3 ∩ Λ_S*`. Exact integer/rational linear algebra (kernel, HNF);
  no hkl enumeration, no modulus bound. `reflections.py` re-exports it as
  `reflection_conditions` (the older grid-sampling reporter survives as
  `reflection_conditions_grid`) and holds equivalent-reflection orbits,
  epsilon factors and multiplicities.
